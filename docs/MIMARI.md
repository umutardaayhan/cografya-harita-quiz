...
3. **`data/` — Paket (DLC) Veri Katmanı** _(bkz. [PAKET_SISTEMI.md](PAKET_SISTEMI.md))_:
   ...
   - `gorseller.js` (ÜRETİLMİŞ): önemli mekânların (Turizm & Kültür Mirası, Dünya Harikaları, Dünya Yapıları) **serbest lisanslı** Wikimedia Commons fotoğrafları. Eşleme `tools/gorsel_kaynaklari.json`'da (kimlik → `dil:Makale` ya da `{"dosya": ...}` elle seçim); `node tools/build_gorseller.js` Wikidata P18 → makale görseli sırasıyla dosyayı bulur, yalnızca CC/kamu malı lisansları kabul eder. Görseller depoya indirilmez, küçük resim adresi + atıf (yazar · lisans · Commons sayfası) saklanır. `project()` onu `item.gorsel` olarak ekler; **yalnızca cevaptan sonra** (hap kartı) ve keşif balonlarında gösterilir, çevrimdışıyken figür kendini gizler.
